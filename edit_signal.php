...
if (($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_SERVER["HTTP_REFERER"]) && strpos(urldecode($_SERVER["HTTP_REFERER"]), urldecode($_SERVER["SERVER_NAME"].$_SERVER["PHP_SELF"])) > 0) && isset($_POST))  {
  $WAFV_Redirect = "edit_signal.php";
  $_SESSION['WAVT_editsignal_710_Errors'] = "";
  if ($WAFV_Redirect == "")  {
  ...
  if ($WAFV_Errors != "")  {
    PostResult($WAFV_Redirect,$WAFV_Errors,"editsignal_710");
  }
}
?>
<?php
...
$colname_rsSignali = "-1";
if (isset($_GET['edit'])) {
  $colname_rsSignali = $_GET['edit'];
}
mysql_select_db($database_iag112new, $iag112new);
$query_rsSignali = sprintf("SELECT signali.*, nug.podelenia.Pod_NameBg,nug. podelenia.Pod_Grad FROM signali, nug.podelenia WHERE signali.id=%s AND signali.pod_id=nug.podelenia.Pod_Id", GetSQLValueString($colname_rsSignali, "int"));
$rsSignali = mysql_query($query_rsSignali, $iag112new) or die(mysql_error());
$row_rsSignali = mysql_fetch_assoc($rsSignali);
$totalRows_rsSignali = mysql_num_rows($rsSignali);
...
if ("" == "") // Trigger
{
  ...
  $WA_table = "logos";
  $WA_sessionName = "edit_signal";
  $WA_redirectURL = "";
  if (function_exists("rel2abs")) $WA_redirectURL = $WA_redirectURL?rel2abs($WA_redirectURL,dirname(__FILE__)):"";
  $WA_keepQueryString = false;
  $WA_fieldNamesStr = "action|ip|username|name|podelenie";
  $WA_fieldValuesStr = "Отваряне на страницата (формата) за редактиране на сигнал №: ".$_GET['edit'] . $WA_AB_Split . "".((isset($_SERVER["REMOTE_ADDR"]))?$_SERVER["REMOTE_ADDR"]:"")  ."" . $WA_AB_Split . "".$_SESSION['username']  ."" . $WA_AB_Split . "".$_SESSION['Name']  ." ".$_SESSION['Familia']  ."" . $WA_AB_Split . "".$_SESSION['Podelenie']  ."";
...
if (($_SERVER["REQUEST_METHOD"] == "POST") && (isset($_SERVER["HTTP_REFERER"]) && strpos(urldecode($_SERVER["HTTP_REFERER"]), urldecode($_SERVER["SERVER_NAME"].$_SERVER["PHP_SELF"])) > 0) && isset($_POST)) // Trigger
{
  $WA_connection = $iag112new;
  $WA_table = "logos";
  $WA_sessionName = "edit_signal_submit";
  $WA_redirectURL = "";
  if (function_exists("rel2abs")) $WA_redirectURL = $WA_redirectURL?rel2abs($WA_redirectURL,dirname(__FILE__)):"";
  $WA_keepQueryString = false;
  $WA_fieldNamesStr = "action|ip|username|name|podelenie";
  $WA_fieldValuesStr = "Успешно изпращане на формата за редакция за сигнал №:". $_GET['edit'] . $WA_AB_Split . "".((isset($_SERVER["REMOTE_ADDR"]))?$_SERVER["REMOTE_ADDR"]:"")  ."" . $WA_AB_Split . "".$_SESSION['username']  ."" . $WA_AB_Split . "".$_SESSION['Name']  ." ".$_SESSION['Familia']  ."" . $WA_AB_Split . "".$_SESSION['Podelenie']  ."";
  $WA_columnTypesStr = "',none,''|',none,''|',none,''|',none,''|',none,''";
  $WA_fieldNames = explode("|", $WA_fieldNamesStr);
  $WA_fieldValues = explode($WA_AB_Split, $WA_fieldValuesStr);
  $WA_columns = explode("|", $WA_columnTypesStr);
   ...
   <div class="row">
      <div class="panel panel-default panel-heading">
		 ...
		 <!-- InstanceBeginEditable name="EditTittle" -->
            <h1>Редактиране на сигна №: <?php echo $row_rsSignali['id']; ?></h1>
            <h3><?php echo $row_rsSignali['Pod_NameBg']; ?></h3>
            <h5>( <?php echo $row_rsSignali['Pod_Grad']; ?> )</h5>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                     <select class="form-control" id="signalfrom" name="signalfrom">
                        <option value="" <?php if (!(strcmp("", $row_rsSignali['signalfrom']))) {echo "selected=\"selected\"";} ?>>Избери от къде е постъпил сигнала</option>
                        <option value="1" <?php if (!(strcmp(1, $row_rsSignali['signalfrom']))) {echo "selected=\"selected\"";} ?>>1. тел. 112</option>
                        <option value="2" <?php if (!(strcmp(2, $row_rsSignali['signalfrom']))) {echo "selected=\"selected\"";} ?>>2. тел. 0800 20 800</option>
                        <option value="3" <?php if (!(strcmp(3, $row_rsSignali['signalfrom']))) {echo "selected=\"selected\"";} ?>>3. Платформа НПО</option>
                     </select>
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "1" . ",") !== false || "1" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Постъпил от" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(1:)
    }
  }
}?>
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                     <input name="signaldate" type="text" class="form-control" id="signaldate" placeholder="Дата на регистриране на сигнала от служителя" value="<?php echo $row_rsSignali['signaldate']; ?>">
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "2" . ",") !== false || "2" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Дата на сигнала" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(2:)
    }
  }
}?>
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                     <input name="identnumber" type="text" class="form-control" id="identnumber" placeholder="Идентификационен номер от републиканския тел. 112" value="<?php echo $row_rsSignali['identnumber']; ?>">
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "3" . ",") !== false || "3" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Идент. №" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(3:)
    }
  }
}?>
                  </div>
...
?>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                     <input name="name" type="text" class="form-control" id="name2" placeholder="Подател на сигнала" value="<?php echo $row_rsSignali['name']; ?>">
                  </div>
               </div>
               <div class="form-group">
                  <label for="phone" class="col-md-3 control-label">Телефон:</label>
                  <div class="col-md-9">
                     <input name="phone" type="text" class="form-control" id="phone" placeholder="Телефонен номер от който е подаден сигнала" value="<?php echo $row_rsSignali['phone']; ?>">
                  </div>
               </div>
               <div class="form-group">
                  <label for="adress" class="col-md-3 control-label">Адрес:</label>
                  <div class="col-md-9">
                     <input name="adress" type="text" class="form-control" id="adress" placeholder="Адрес на подателя на сигнала" value="<?php echo $row_rsSignali['adress']; ?>">
                  </div>
               </div>
               <div class="form-group">
                  <label for="opisanie" class="col-md-3 control-label">Описание:</label>
                  <div class="col-md-9">
                     <textarea name="opisanie" class="form-control" id="opisanie" placeholder="Описание на сигнала"><?php echo $row_rsSignali['opisanie']; ?></textarea>
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "10" . ",") !== false || "10" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Описание" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(10:)
    }
  }
}?>
                  </div>
...
do {  
?>
                        <option value="<?php echo $row_rsNarushenia['nid']?>"<?php if (!(strcmp($row_rsNarushenia['nid'], $row_rsSignali['narushenia']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsNarushenia['naimenovanie']?></option>
                        <?php
...
?>
                     ...
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "9" . ",") !== false || "9" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Вид нарушение" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(9:)
    }
  }
}?>
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9"><!--Сигналът е за-->
                     <select class="form-control" id="send_to" name="send_to">
                       <option value="" <?php if (!(strcmp("", $row_rsSignali['send_to']))) {echo "selected=\"selected\"";} ?>>Избери на кого е предадено съобщението</option>
                       <?php
do {  
?>
<option value="<?php echo $row_rsPodelenia['Pod_Id']?>"<?php if (!(strcmp($row_rsPodelenia['Pod_Id'], $row_rsSignali['send_to']))) {echo "selected=\"selected\"";} ?>><?php echo $row_rsPodelenia['Pod_NameBg']?></option>
                       <?php
...
?>
                       <option value="8888" <?php if (!(strcmp(8888, $row_rsSignali['send_to']))) {echo "selected=\"selected\"";} ?>>ИАРА</option>
                     </select>
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "4" . ",") !== false || "4" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Предадено на" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(4:)
    }
  }
}?>
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                    <input  <?php if (!(strcmp($row_rsSignali['send_to_extra'],"0"))) {echo "checked=\"checked\"";} ?> type="radio" name="send_extra" id="send_extra" value="0" checked />На никой
                    <input  <?php if (!(strcmp($row_rsSignali['send_to_extra'],"1"))) {echo "checked=\"checked\"";} ?> type="radio" name="send_extra" id="send_extra" value="1" />Полиция
                    <input  <?php if (!(strcmp($row_rsSignali['send_to_extra'],"2"))) {echo "checked=\"checked\"";} ?> type="radio" name="send_extra" id="send_extra" value="2" />Пожарна
                    <input  <?php if (!(strcmp($row_rsSignali['send_to_extra'],"3"))) {echo "checked=\"checked\"";} ?> type="radio" name="send_extra" id="send_extra" value="3" />БАБХ
                  </div>
               </div>
               <div class="form-group">
                  <label for="deistvie" class="col-md-3 control-label">Предприети действия:</label>
                  <div class="col-md-9">
                     <textarea rows="2" class="form-control" placeholder="Предприети действия от регистриращия сигнала" name="deistvie" cols="50" id="deistvie"><?php echo $row_rsSignali['deistvie']; ?></textarea>
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "6" . ",") !== false || "6" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Предприети действия" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(6:)
    }
  }
}?>
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                     <input name="deistvie_date" type="text" class="form-control" id="deistvie_date" placeholder="Дата на предприетите действия от служителя приел сигнала" value="<?php echo $row_rsSignali['deistvie_date']; ?>">
                     <?php
if (ValidatedField('editsignal_710','editsignal_710'))  {
  if ((strpos((",".ValidatedField("editsignal_710","editsignal_710").","), "," . "7" . ",") !== false || "7" == ""))  {
    if (!(false))  {
?>
                        <span class="text-danger">Полето "Дата на действието" е задължително</span>
                        <?php //WAFV_Conditional add_message.php editsignal_710(7:)
    }
  }
}?>
                  </div>
               ...
               <div class="form-group">
                  ...
                  <div class="col-md-9">
                     <textarea rows="4" class="form-control" placeholder="Забележка" name="notes" cols="50" id="notes"><?php echo $row_rsSignali['notes']; ?></textarea>
                  </div>
               </div>
               <div class="form-group">
                  <label for="policia" class="col-md-3 control-label">Сигнала е предаден на полицията след 22 часа</label>
                  <div class="col-md-9">
                     <input <?php if (!(strcmp($row_rsSignali['policia'],1))) {echo "checked=\"checked\"";} ?> id="policia" class="checkbox1" name="policia" type="checkbox" value="1">
                  </div>
               </div>
               <div class="form-group">
                  <div class="col-md-offset-3 col-md-9">
                     <input class="btn btn-info" type="submit" value="Редактиране на сигнала">
                  </div>
               </div>
            </form>
            <!-- InstanceEndEditable -->
